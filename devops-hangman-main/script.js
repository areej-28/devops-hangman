...
function updateWrongLetters() {
    ...
    if (wrong.length === 0) {
        ...
        wrongLettersDiv.textContent = wrong.join(', ');  // ✅ FIXED! Use 'wrong' not 'guessedLetters'
    }
}
...
function gameWon() {
    ...
    if (gameState.currentPlayer === 1) {
        gameState.player1.score += 10;  // ✅ FIXED! Give to player 1
        document.getElementById('score1').textContent = gameState.player1.score;
    } else {
        gameState.player2.score += 10;  // ✅ FIXED! Give to player 2
    ...
    const winnerName = gameState.currentPlayer === 1 ? 
        gameState.player1.name : gameState.player2.name;  // ✅ FIXED! Show correct winner
